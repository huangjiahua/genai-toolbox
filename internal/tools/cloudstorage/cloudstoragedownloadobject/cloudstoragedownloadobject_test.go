...
func TestParseFromYamlCloudStorageDownloadObject(t *testing.T) {
	...
	tcs := []struct {
	...
	}{
		{
			...
			in: `
			kind: tool
			name: download_object_tool
			type: cloud-storage-download-object
			source: my-gcs
			description: Download a Cloud Storage object to local disk
			`,
			want: server.ToolConfigs{
				"download_object_tool": cloudstoragedownloadobject.Config{
					Name:         "download_object_tool",
					Type:         "cloud-storage-download-object",
					Source:       "my-gcs",
					Description:  "Download a Cloud Storage object to local disk",
		...
		},
		{
			...
			in: `
			kind: tool
			name: secure_download_object
			type: cloud-storage-download-object
			source: prod-gcs
			description: Download object with authentication
			authRequired:
				- google-auth-service
				- api-key-service
			`,
			want: server.ToolConfigs{
				"secure_download_object": cloudstoragedownloadobject.Config{
					Name:         "secure_download_object",
					Type:         "cloud-storage-download-object",
					Source:       "prod-gcs",
					Description:  "Download object with authentication",
