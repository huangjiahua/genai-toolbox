...
type compatibleSource interface {
	DownloadObject(ctx context.Context, bucket, object, dest string) (map[string]any, error)
}
...
func (cfg Config) Initialize(srcs map[string]sources.Source) (tools.Tool, error) {
	...
	destinationParam := parameters.NewStringParameter(destinationKey, "Local filesystem path where the object will be written. Relative paths are resolved against the toolbox server's working directory.")
	params := parameters.Parameters{bucketParam, objectParam, destinationParam}
...
func (t Tool) Invoke(ctx context.Context, resourceMgr tools.SourceProvider, params parameters.ParamValues, accessToken tools.AccessToken) (any, util.ToolboxError) {
	...
	resp, err := source.DownloadObject(ctx, bucket, object, destination)
	if err != nil {
